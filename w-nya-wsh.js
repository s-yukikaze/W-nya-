  ...
  while (T.length) {
    ...
    if (E.match(/^[w]+/)) {
      ...
      for (;;) {
        ...
        S.unshift("G[" + G++ + "]");
        if (!T.length || !T[0].match(/([W‚v]+)([w‚—]+)/)) break;
        T.shift();
      }
    }
  }
...
function W_nya_ToJavaScript(I) {
  var I = (I.match(/Ö|®|ƒ‡/g) || []).join("").replace(/Ö/g,"W").replace(/®/g,"w").replace(/ƒ‡/g,"v");
...
if (WScript.Arguments.Unnamed.Length > 0) {
	...
	strm.Charset = "UTF-8";
	strm.Open();
	strm.LoadFromFile(WScript.Arguments.Unnamed.Item(0));
	var u_nya_ = strm.ReadText();
	strm.Close();
} else {
	var u_nya_ = WScript.StdIn.ReadAll();
}

eval(W_nya_ToJavaScript(u_nya_) + "function O(x) { WScript.StdOut.Write(String.fromCharCode(x)); }");
